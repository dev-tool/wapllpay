...
Webllpay.prototype._buildRequestForm = function(para_temp,method,button_name,tip_name) {
	...
	var sHtml = "<form style='width:100%;text-align:center;padding-top:50px;' id='llpaysubmit' name='llpaysubmit' action='" + url.llpay_gateway_new + "' method='" + method + "'>";
 ...
 * 银行卡卡 bin 查询接口
 */
Webllpay.prototype.queryBankCard= function(json, cb){
  var self = this;
	var defaultValue = util.clone(defaultKeysValue());
	var cloneConfig = util.clone(this.config);
	...
	json = util.pick(json,['oid_partner','sign_type','sign','card_no','pay_type','flag_amt_limit']);//拾取连连支付所支持的参数
	var obj = this._buildRequestPara(json);
	var request = require('request');
	request.post({url:url.query_bankcard_url,body:obj},function(error, response, body){
